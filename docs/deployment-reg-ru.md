...
**Python на REG.RU:** системный `python3` часто **3.8** с **pip 9** — скрипт `make install-backend` сам обновит pip и создаст venv. Нужен **Python >= 3.8**. Проверка (каждую команду отдельно):
...
Если есть более новый Python — установка подхватит его автоматически. Иначе достаточно `python3` 3.8.6:
